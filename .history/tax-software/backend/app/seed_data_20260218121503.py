...
def ai_generate_credit_report():
    ...
    return {
        ...
        "trade-lines": ["Auto Loan", "Credit Card"],
...
def insert_credit_report(conn, user_id, credit_report):
    conn.execute("INSERT INTO credit_reports (user_id, score, trade-lines, derogatory, created_at) VALUES (?, ?, ?, ?, ?)",
        (user_id, credit_report["score"], str(credit_report["trade-lines"]), str(credit_report["derogatory"]), datetime.now()))
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        print("CADE2 Issues:", cade2_pre_submission_check(tax_profile))
