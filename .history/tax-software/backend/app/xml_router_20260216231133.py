...
def upload_xml(file: UploadFile = File(...), token: str = Depends(bearer_auth)):
    path = os.path.join(XML_STORAGE, file.filename)
    with open(path, "wb") as f:
        f.write(file.file.read())
    logger.info(f"AUDIT: XML uploaded: {file.filename}")
...
def download_xml(filename: str, token: str = Depends(bearer_auth)):
    ...
    if not os.path.exists(path):
        logger.warning(f"AUDIT: XML download failed: {filename} not found")
