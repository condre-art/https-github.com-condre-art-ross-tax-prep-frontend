...
export default function IRSEfileWidget() {
  ...
  return (
    <Card title="IRS E-file Certificate">
      <div className="irs-efile-controls">
        <input
          className="input"
          placeholder="ERO ID"
          value={eroId}
        ...
        <button className="button irs-efile-get" onClick={handleGet} disabled={loading || !eroId}>
          Get Cert
        </button>
      </div>
      {loading && <div>Loading...</div>}
      {error && <div className="irs-efile-error">{error}</div>}
      {cert && (
        <pre className="irs-efile-cert">{JSON.stringify(cert, null, 2)}</pre>
      )}
    </Card>
  );
}
