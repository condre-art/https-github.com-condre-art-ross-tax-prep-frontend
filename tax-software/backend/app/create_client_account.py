...
def create_client():
    # Use a simple integer for demo DB (id INTEGER PRIMARY KEY)
    user_id = 1001  # Pick a value not in use
    ...
    cur.execute(
        """
        INSERT OR REPLACE INTO users (id, tenant_id, email, password_hash, role, status, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
